...
**一句话总结 hashCode 与 equals 的关系**：`hashCode` 与 `equals` 紧密相关 —— 重写 `equals` 时必须重写 `hashCode`，以保证在哈希表等数据结构中对象的相等性判断与存储/查找操作正常；重写 `hashCode` 时需确保**相等的对象具有相同的哈希码**，但**相同哈希码的对象不一定相等**（哈希冲突，详见 [[wiki/编程/java基础/object/== 与 equals 有什么区别？]]）。
...
> 父类引用指向子类对象时，`getClass()` 返回的是子类 `Dog`（对应于 [[wiki/编程/java基础/面向对象/01 继承 封装 多态]] 中的向上转型）。
...
- [[wiki/编程/java基础/面向对象/01 继承 封装 多态]] — getClass 与运行时多态
